...
test("promise with expected error", async () => {
  const expected = [
  ...
  await confirmPromiseOperationWithError();
...
test("promise with unexpected error in db.run()", async () => {
  const db = createDb();
  ...
  try {
    await confirmPromiseOperationWithError(db);
...
test("promise with unexpected error in db.get()", async () => {
  const expected =
    "Expected error in db.run(): SQLITE_CONSTRAINT: NOT NULL constraint failed: books.title";
